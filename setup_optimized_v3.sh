...
check_sudo() {
    ...
    if [ "${EUID}" -ne 0 ]; then
        echo_error "This script needs to be run with root privileges. Please use 'sudo ./setup_optimized_v3.sh'."
        exit 1
    fi
...
echo_info "=================================================="
echo_info " S4V Camera Setup Script - Enhanced for Usability "
echo_info "=================================================="
...
if [ ! -f "${NGINX_CONFIG_FILE}" ]; then
    echo_error "Nginx configuration file ('${NGINX_CONFIG_FILE}') not found in the current directory ('$(pwd)')."
    echo_error "Please make sure '${NGINX_CONFIG_FILE}' is in the same directory as this script."
    exit 1
fi

if [ -f "${NGINX_CONFIG_PATH}" ]; then
    echo_warn "An existing Nginx configuration file was found at '${NGINX_CONFIG_PATH}'."
    if confirm_action "Do you want to replace it with '${NGINX_CONFIG_FILE}'? A backup of the current file will be created."; then
        ...
        echo_info "Copying new Nginx configuration from '${NGINX_CONFIG_FILE}' to '${NGINX_CONFIG_PATH}'..."
    ...
    echo_info "No existing Nginx configuration found. Copying '${NGINX_CONFIG_FILE}' to '${NGINX_CONFIG_PATH}'..."
    cp "${NGINX_CONFIG_FILE}" "${NGINX_CONFIG_PATH}"
...
if [ ! -f "${SYSTEMD_SERVICE_FILE}" ]; then
    echo_error "Systemd service file ('${SYSTEMD_SERVICE_FILE}') not found in the current directory ('$(pwd)')."
    echo_error "Please make sure '${SYSTEMD_SERVICE_FILE}' is in the same directory as this script."
    exit 1
fi

if [ -f "${SYSTEMD_SERVICE_PATH}" ]; then
    echo_warn "An existing systemd service file was found at '${SYSTEMD_SERVICE_PATH}'."
    if confirm_action "Do you want to replace it with '${SYSTEMD_SERVICE_FILE}' from the current directory?"; then
        echo_info "Copying new systemd service file to '${SYSTEMD_SERVICE_PATH}'..."
...
if systemctl is-active --quiet "${SYSTEMD_SERVICE_FILE}"; then
    echo_info "S4V Camera service (${SYSTEMD_SERVICE_FILE}) is active and running."
    CAMERA_SERVICE_ACTIVE=true
else
    echo_warn "S4V Camera service (${SYSTEMD_SERVICE_FILE}) may not be running correctly."
    echo_warn "Check with: sudo systemctl status ${SYSTEMD_SERVICE_FILE}  OR  sudo journalctl -u ${SYSTEMD_SERVICE_FILE}"
fi
