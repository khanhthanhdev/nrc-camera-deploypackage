...
PI_WEBRTC_EXECUTABLE_NAME="pi_webrtc"
...
check_sudo() {
    ...
    if [ "${EUID}" -ne 0 ]; then
        echo_error "This script needs to be run with root privileges. Please use \'sudo ./setup_optimized_v5.sh\'."
        exit 1
    fi
...
echo_info "=================================================="
echo_info " S4V Camera Setup Script - v5 (Dual Source Install)"
echo_info "=================================================="
echo_info "This script will install components from b4iterdev (UI/Server) and TzuHuanTai (pi_webrtc executable)."
...
essential_commands=("wget" "unzip" "tar" "openssl" "systemctl" "apt" "grep" "uname" "dpkg" "mkdir" "rm" "cp" "hostname" "sed")
missing_commands=0
...
if [ "$missing_commands" -ne 0 ]; then
    echo_error "Please install the missing commands and try again. For 	'unzip	' and 	'tar	', use 	'sudo apt install unzip tar	'."
    exit 1
fi
...
tar -xzf "${TZUHUANTAI_ARCHIVE_NAME}" -C "${TZUHUANTAI_TEMP_DIR}" --strip-components=1 # Assuming tarball has a single top-level dir
echo_info "Extraction complete."

# Find and copy pi_webrtc executable
FOUND_PI_WEBRTC=$(find "${TZUHUANTAI_TEMP_DIR}" -name "${PI_WEBRTC_EXECUTABLE_NAME}" -type f -executable)
if [ -n "${FOUND_PI_WEBRTC}" ]; then
    echo_info "Found pi_webrtc executable at: ${FOUND_PI_WEBRTC}"
    echo_info "Copying ${PI_WEBRTC_EXECUTABLE_NAME} to ${S4V_DIR}/ ..."
    cp "${FOUND_PI_WEBRTC}" "${S4V_DIR}/${PI_WEBRTC_EXECUTABLE_NAME}"
    chmod +x "${S4V_DIR}/${PI_WEBRTC_EXECUTABLE_NAME}" # Ensure it's executable
    echo_info "${PI_WEBRTC_EXECUTABLE_NAME} installed successfully."
else
    echo_error "Could not find the executable 	'${PI_WEBRTC_EXECUTABLE_NAME}	' within the extracted TzuHuanTai package."
